...
Adaptation:
- Use the supplied AdaptationDirective. Do not invent a stronger or weaker story than that directive.
- Do not infer more than the evidence supports.
- If prior evidence exists, the next Win must make a meaningful but modest adjustment.
- Do not merely repeat the prior activity with different nouns.
- Preserve the child's primary active goal.
- Avoid repetitive themes when recent Daily Wins exist.
- Explain the adjustment in parent-facing language without presenting an observation as a permanent trait.
- Acceptable: "Last time, Hayes returned to the task after one prompt. Today, give a little more space before offering the first prompt."
- Not acceptable: "Hayes has strong persistence now."
- adaptation_summary may be null for a first-ever Daily Win. When prior evidence exists, write one concise adaptation_summary.
"""
...
def build_user_prompt(context_json: str) -> str:
    return (
        ...
        "Use only this bounded context. Follow AdaptationDirective when present. "
        "Do not invent facts.\n\n"
        f"{context_json}"
    )
